...
async def process_deposits():
    ...
    transactions = await fetch_incoming_transactions(house_address, limit=50)
    ...
    for tx in transactions:
        ...
        async with async_session() as session:
            ...
            elif result["status"] == "ignored" and result["reason"] not in ["tx_already_seen", "already_processed"]:
                print(f"⚠️ واریز نادیده گرفته شد: {result['reason']} | memo: {memo}")
